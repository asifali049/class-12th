      ...
      {numericals.map((num, index) => (
        <div key={num.id} className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${difficultyColors[num.difficulty || 'medium']}`}>
              {num.difficulty === 'hard' ? 'Board Level' : (num.difficulty || 'Medium')}
            </div>
          </div>
          ...
          <div className="space-y-6">
            {/* If we have Biology style simple solution */}
            {num.solution && (
              <div className="bg-slate-50 dark:bg-slate-900 p-5 rounded-xl border border-slate-100 dark:border-slate-800">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-4">Solution</span>
                <div className="text-slate-700 dark:text-slate-300 text-lg leading-relaxed space-y-2">
                  {typeof num.solution === 'string' ? (
                     num.solution.split('\n').map((line: string, i: number) => (
                       <div key={i}>{renderMathLine(line)}</div>
                     ))
                  ) : (
                    <BilingualDisplay content={num.solution as any} language={language} />
                  )}
                </div>
              </div>
            )}

            {/* If we have Physics style detailed blocks */}
            {(num.given || num.required) && (
            ...
            {(num.substitution || num.calculation) && (
