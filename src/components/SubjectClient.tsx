...
export default function SubjectClient({ 
  ...
  return (
    <main className="min-h-screen bg-slate-50 md:bg-slate-100 flex flex-col md:flex-row pb-20 md:pb-0">
      
      {/* Sidebar - Chapter List */}
      <aside className="hidden md:flex w-80 bg-white border-r border-slate-200 flex-col h-screen sticky top-0">
        <div className="p-6 border-b border-slate-100">
          <Link href="/" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-blue-600 transition-colors mb-4">
            <ArrowLeft className="w-4 h-4" /> Back to Dashboard
          </Link>
          <h2 className="text-xl font-bold text-slate-900">{subject.name}</h2>
          <p className="text-xs text-slate-500 mt-1 font-medium">UP Board Session 2026-27</p>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        {/* Topbar */}
        <header className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 z-40 shrink-0">
          <div className="flex items-center gap-3">
            <Link href="/" className="md:hidden text-slate-500">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div>
              <h1 className="text-sm md:text-base font-bold text-slate-900 truncate max-w-[200px] md:max-w-md hidden md:block">
                {activeChapter.name} {activeChapter.hindiName && `(${activeChapter.hindiName})`}
              </h1>
              
              {/* Mobile Chapter Dropdown */}
              <select 
                className="md:hidden text-sm font-bold text-slate-900 bg-transparent border-none focus:ring-0 cursor-pointer max-w-[200px] truncate"
              ...
              <div className="flex items-center gap-1 text-[10px] font-semibold text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full border border-green-100 w-fit mt-0.5">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <div className="hidden sm:flex bg-slate-100 p-1 rounded-lg border border-slate-200">
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-md transition-colors ${language === 'Hindi' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                हिंदी
              </button>
              <button 
                onClick={() => setLanguage("Hinglish")}
                className={`px-3 py-1 text-xs font-bold rounded-md transition-colors ${language === 'Hinglish' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Hinglish
              </button>
            </div>
            <button className="text-slate-400 hover:text-slate-700 transition-colors hidden sm:block"><Search className="w-5 h-5" /></button>
            <button className="text-slate-400 hover:text-slate-700 transition-colors hidden sm:block"><Share2 className="w-5 h-5" /></button>
            <button className="text-slate-400 hover:text-slate-700 transition-colors"><Bookmark className="w-5 h-5" /></button>
            <button className="text-slate-400 hover:text-slate-700 transition-colors hidden sm:block"><Maximize2 className="w-5 h-5" /></button>
            <ThemeToggle />
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-100 relative">
          ...
          <motion.div 
            ...
            className="w-full max-w-[860px] mx-auto bg-white min-h-[1056px] shadow-sm md:shadow-md rounded-none md:rounded-lg notebook-bg notebook-margin overflow-hidden relative"
          >
